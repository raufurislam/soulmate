...
const Banner = () => {
  ...
  return (
    <div className="max-w-screen-xl mx-auto pt-4 lg:px-2 px-4 ">
      <div className="rounded-lg bg-neutral lg:px-12 px-4 md:px-7 py-5">
        <Swiper
          loop={true}
          spaceBetween={30}
          centeredSlides={true}
          ...
          {slides.map((slide, index) => (
            <SwiperSlide key={slide.id}>
              <div className="flex flex-col-reverse md:flex-row gap-8 items-center">
                ...
                >
                  <h1 className="lg:text-4xl md:text-3xl text-2xl mb-3 text-text1 font-bold">
                    {slide.title}
                  </h1>
                  <p className="lg:w-2/3 text-sm text-text2">
                    {slide.description}
                  </p>
                  <button className="text-white bg-primary hover:bg-[#d64a5b] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mt-6">
                    Explore Now
                  </button>
                </div>
                {/* Image */}
